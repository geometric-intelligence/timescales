...
def build_model(
    ...
    """Build an untrained RNN for null-task analysis."""
    return RNN(
        input_size=1, hidden_size=n_neurons, output_size=1, dt=dt,
        time_constants_config={"type": "discrete", "values": [tau]},
        learn_time_constants=False, activation=activation, wrec_init=wrec_init,
        recurrent_gain=g, zero_diag_wrec=False, dynamics_type=dynamics_type,
        alpha_parameterization="linear", noise_std=0.0,
    )


def get_W(model: RNN) -> NDArray:
...
def generate_W(n_neurons: int, wrec_init: str = "normal_scaled") -> NDArray:
    """Generate W via RNN initialization (default: Gaussian N(0,1/N))."""
...
def generate_W_levy(
    ...
    normalization="rnn"    → scale = 1/sqrt(N)
                             Matches RNN wrec_init='levy_stable'.
...
# - **`"rnn"`**: scale $= 1/\sqrt{N}$ — matches `RNN wrec_init='levy_stable'`.
